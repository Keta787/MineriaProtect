...
| Integrado (cruce) | `empleos` = experiencias de JobHop enriquecidas con ESCO (1 fila = 1 experiencia; unión N:1) |
...
| `empleos.csv` (integrado) | — | 1.506.445 × 11 (inmutable) | Integración validada; cruce N:1 por persona |
| `empleos_limpio.csv` (integrado) | 1.506.445 × 11 | **1.506.434 × 14** | Pipeline en 7 fases: −11 filas con fechas futuras (MCAR) + 3 banderas `es_*` |
| `empleos_limpio_sin_outliers.csv` | 1.506.434 × 14 | 1.364.853 × 14 | Variante sin cola larga (`dur_Q ≥ 25`, IQR de Tukey); −141.581 filas / 277.052 personas |
...
│       ├── empleos_limpio.csv      # 1.506.434 × 14 (limpio final, banderas es_*)
│       └── empleos_limpio_sin_outliers.csv  # 1.364.853 × 14 (sin cola larga)
...
- **Salida única en CSV para el cruce:** `empleos.csv`, `empleos_limpio.csv` y `empleos_limpio_sin_outliers.csv` se mantienen exclusivamente en CSV (`data/03_processed/`), regenerables desde los scripts. Los CSV no se versionan por tamaño (156–184 MB) y la estructura de la carpeta se conserva en git con `.gitkeep`.
- **Banderas en lugar de imputación:** la ausencia es informativa; no se usa `fillna`. Nulos estructurales conservados con banderas: `es_unknown_ocupacion` (104.993), `es_rescatado` (10.176) y re-categorización de `'None'` → `'No reportado'` (174.036).
- **Censura (`Present`) explícita:** `es_vigente` (76.180) marca empleos vigentes al capturar el CV; no se inventa fecha de fin (censura por la derecha).
...
- **Solo se elimina lo justificado:** 11 filas con fechas futuras (MCAR de dominio, <1 % de las filas). La cola larga de duración (IQR de Tukey, límite superior **24,5**) se **conserva**: estabilidad de carrera = señal, no contaminación.
...
| D2 | Empleos simultáneos / pluriempleo (74.357 claves cortas) | ¿Cómo se concreta la regla de negocio "transición" en secuencias con pluriempleo conservado? |
...
- `data/03_processed/empleos.csv` es el integrado canónico e inmutable; si falta, se restaura desde el historial git y se re-exporta a CSV (no se reconstruye con el script).
- El repositorio versiona `data/01_raw/` y `data/02_interim/` completos; no versiona los CSV del cruce por tamaño.

## Estado actual y siguientes pasos

**Estado:** la limpieza está **100 % terminada y verificada** en los dos niveles — fuentes (`limpiar_datos.py`) e integrado (`limpiar_empleos.py`). `empleos_limpio.csv` alcanzó **1.506.434 filas × 14 columnas** (284.247 personas), con validación final 6/6; la variante sin cola larga (1.364.853 × 14, 277.052 personas) queda lista para comparar. Alineado con las sesiones 1–3: acta (1.0), fuentes/carga defensiva/EDA/API (2.0) y pipeline de limpieza con rúbrica de código (3.0).
...
- **Pluriempleo conservado:** las 74.357 claves cortas simultáneas se mantienen; la regla de "transición" se define en la fase de transformación (D2).
