...
>   **trayectorias laborales**. El Observatorio Laboral para la Educación (OLE)
>   colombiano queda solo como contexto o fuente complementaria opcional.
...
> - El formato final es **exclusivamente CSV** para el integrado y sus limpios
>   (`data/03_processed/empleos.csv`, `empleos_limpio.csv` y
>   `empleos_limpio_sin_outliers.csv`), según decisión documentada del equipo.
>   Los parquet del cruce fueron eliminados; su historial queda en git.
...
|---|---|---|---|
| **JobHop v2** (`aida-ugent/JobHop`) | `data/01_raw/JobHop_v2_train.parquet` → limpio en `data/02_interim/JobHop_v2_train_limpio.parquet` | Fuente principal: trayectorias (quién, cuándo, ocupación por código, nivel educativo) | ✅ Seleccionada |
| **ESCO v1.2.1** — occupations | `data/01_raw/ESCO/occupations_en.csv` → `data/02_interim/ESCO/occupations_en_limpio.csv` | Traduce `matched_code → nombre de ocupación + grupo ISCO-08` | ✅ Seleccionada |
| **ESCO v1.2.1** — ISCOGroups | `data/01_raw/ESCO/ISCOGroups_en.csv` → `data/02_interim/ESCO/ISCOGroups_en_limpio.csv` | Etiqueta y jerarquía del área ocupacional (`code` 4 dígitos → `label` + nivel) | ✅ Seleccionada |
| **OLE Colombia** | *(no está en el repo)* | Contexto colombiano **opcional**; sin clave de unión individual con JobHop | ❌ Descartado como fuente principal |
...
| Limpio sin cola larga (`empleos_limpio_sin_outliers.csv`) | **1.364.853 × 14**, 277.052 personas, `dur_Q` máx. 24 | `dividir_por_outliers.py` |
| Cruce N:1 (nunca multiplica filas) | 1.506.445 filas; la integración no introduce duplicados | Auditoría §5 |
| Duplicados exactos / por PK | 0 exactos; 74.357 por PK (`resume_id`, `start`, `end`) **conservados** (pluriempleo legítimo) | Auditoría §6 |
| Duración trimestral **inclusiva** (`end − start + 1`) | mediana **5**, Q1 2, Q3 11, máx 160 | Auditoría §8 |
| IQR (Tukey) de duración | IQR 9, límite superior 24,5; 141.581 filas fuera (todas por arriba) — **conservadas** | Auditoría §8 |
...
|---|---|---|---|
| **1. Comprensión del negocio** | **1. Selección** | 🟢 **Hecho** | Objetivo, pregunta de investigación, fuentes seleccionadas (JobHop v2 + ESCO v1.2.1) y descarte justificado de OLE |
...
consolidadas en la documentación del repositorio (actualmente
...
- **Enriquecimiento con ESCO:** junto a `occupations` e `ISCOGroups`, este
  repositorio versiona el resto de la taxonomía (`skills`, `greenShareOcc`,
  `occupationSkillRelations`) en `data/01_raw/ESCO/` y sus limpias en
  `data/02_interim/ESCO/`. Su uso pleno queda reservado a una fase posterior
  (deuda D3), si la pregunta de investigación lo exige: hoy alimentan el cruce
  solo `occupations` e `ISCOGroups`.
- **OLE Colombia:** descartado como fuente principal (sin clave de unión
  individual con JobHop; granularidad insuficiente).

**Hitos entregables:**
- `README_ACTUALIZADO.md` con objetivo, pregunta de investigación (sencilla y
  técnica), árbol del proyecto, reglas de oro (originales intocables, salida única
  CSV, idempotencia) y bitácora de cambios.
- Decisiones de negocio documentadas (tabla de fuentes §2.2, cifras ancla §2.3 y
  la estrategia de banderas/imputación en §4.3).
...
- **Outliers:** no existe "edad 220 / 600 GB"; el integrado no tiene columna
  ...
  (IQR: Q1 2, Q3 11, límite 24,5 → 141.581 filas / 97.947 personas por arriba) se
  conserva: **estabilidad = señal**.

**Hitos entregables:**
- Auditoría con diagnóstico de calidad, traslapes (154.131 personas con al menos
  un solape estricto, 82 % del total) y categorías canónicas (3/5/426/2.966, 0
  colisiones).
...
**idempotencia byte a byte** verificada. El parquet de JobHop también pasa por
este proceso (`data/01_raw/JobHop_v2_train.parquet` →
`data/02_interim/JobHop_v2_train_limpio.parquet`), con lineage reproducible.
...
- **Derivado para comparación:** `empleos_limpio_sin_outliers.csv` (1.364.853 ×
  14, 277.052 personas, `dur_Q` máx. 24) generado por
  `src/limpieza/dividir_por_outliers.py` con el mismo criterio (`dur_Q ≥ 25`),
  para contrastar el impacto de la cola larga antes de elegir la técnica de
  minería.

**Hitos entregables:**
- `src/limpieza/limpiar_empleos.py` (clase `LimpiadorEmpleos`, 7 fases, asserts
  por fase, docstrings en español bajo A9) y `src/limpieza/limpiar_datos.py`
  (limpieza de fuentes).
- `src/limpieza/dividir_por_outliers.py` (split de la cola larga con bitácora en
  `logs/bitacora_dividir_outliers.csv`).
- `notebooks/Diagnostico_Limpieza_Empleos.ipynb` reproduciendo el pipeline con
  evidencia impresa (0 errores).
- `data/03_processed/empleos_limpio.csv` y `empleos_limpio_sin_outliers.csv`
  regenerables y verificados (formato CSV).
- `requirements.txt` en la raíz (pandas≥2, pyarrow≥14).
...
5. Definir el **destino de datos procesados** (deuda formal **D4**): en el layout
   actual los procesados viven en `data/03_processed/`, manteniendo el formato CSV
   como estándar y el original intocable.
...
1. Resolver las **deudas formales heredadas de la fase de diseño** antes de
   ...
   - **D2** — empleos simultáneos (**traslapes**: 154.131 personas con ≥1 solape
     estricto) y claves cortas conservadas (74.357/9.601) en las transiciones:
     cómo se concreta la regla de negocio "transición" en secuencias con
     pluriempleo.
...
5. Describir cómo el conocimiento producido **contextualiza futuras preguntas**
   (p. ej. relación formación–empleo) y qué quedaría descartado por limitación de
   fuentes (OLE sigue sin clave de unión individual; no hay causalidad).
...
1. **Transformación formal** de experiencias a secuencias por persona (`Sₚ`),
   ...
   simultáneos) y **D4** (destino de procesados en `data/03_processed/`, formato
   CSV).
...
(Documentación)** del estándar interno de buenas prácticas del proyecto
(`BUENAS_PRACTICAS_CODIGO.md`, documento base de sesiones previas, no versionado
en este repositorio):
...
1. `README_ACTUALIZADO.md` — configuración actual del proyecto: objetivo,
   pregunta de investigación, layout nuevo (01_raw/02_interim/03_processed), CSV
   del cruce y bitácora de cambios.
2. `AUDITORIA_RAMA_PRUEBA.md` — documento base de sesiones previas (no versionado
   en este repositorio): validación de integración (V1–V7), cifras del diagnóstico
   y del pipeline de limpieza (`empleos_limpio`), decisiones pendientes D1–D4 y
   migración a CSV.
3. `BUENAS_PRACTICAS_CODIGO.md` — documento base del estándar de calidad,
   estructura, nomenclatura y reproducibilidad (no versionado en este
   repositorio; contiene la sección A9 de documentación).
4. `contexto_sesion3_mineriaprotect.md` — documento base con la teoría aplicada
   (MCAR/MAR/MNAR, IQR, censura) y decisiones de limpieza justificadas (se
   conserva en disco, no versionado).
...
Europea) · OLE Colombia (contexto opcional).