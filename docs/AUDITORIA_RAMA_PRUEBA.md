...
> Los documentos viven en `README.md` (raíz, puerta de entrada) y en `docs/` (este documento,
...
|---|---|---|
| Reorganización | Estructura actual: `data/{original, limpia, cruce}`, `notebooks/`, `docs/`, `src/` | ✅ Commit `3698044` (historial preservado con `git mv`); reestructuración posterior revisada en §14 |
...
| Limpieza de `empleos` (bloque 4) | Pipeline ejecutado, cifras reconciliadas contra los datos | ✅ `empleos_limpio.csv` generado y verificado (formato CSV, §16) §13 |
| Pendiente principal | `src/limpieza/limpiar_empleos.py` → `empleos_limpio` | ✅ Ejecutado; los pendientes de minería quedan en §12 |
...
├── README.md
├── .gitignore
├── requirements.txt                # dependencias: pandas>=2, pyarrow>=14
├── data/                           # datos (original/ y limpia/ en disco y versionadas; cruce/ en CSV)
...
│   └── cruce/                      # empleos.csv + empleos_limpio.csv (formato CSV; no versionados por tamaño)
├── notebooks/                      # cuadernos: Lectura, Diagnostico_Limpieza_Empleos, presentacion (análisis, §15)
├── docs/                           # documentación: AUDITORIA, BUENAS_PRACTICAS_CODIGO, METODOLOGIA_KDD_CRISPDM, contexto
├── src/
...
│       └── limpiar_empleos.py      # pipeline bloque 4 → data/cruce/empleos_limpio.csv
```
...
— y `data/cruce/*.csv` (el cruce se trabaja y mantiene en **CSV**, sin versionar por tamaño; se
regenera desde los scripts y el historial git — ver §16). `data/limpio/`, `docs/historicos/` y los
ESCO descartados fueron **borrados** de disco. El integrado (`empleos.csv`) se regenera desde el
historial git (los parquet del cruce fueron eliminados por decisión; el formato acordado es CSV).

Reproducción:
```text
python src/limpieza/limpiar_datos.py     # fuente: data/original/ → data/limpia/ (limpieza de fuentes; JobHop ya limpio, se omite)
python src/limpieza/limpiar_empleos.py   # integrado → data/cruce/empleos_limpio.csv (bloque 4, §13)
# integración: empleos.csv existe en data/cruce (el integrado no se regenera con este script).
# cruzar_datos.py fue ELIMINADO del repo por decisión; se recupera del historial git si hace falta regenerar empleos.csv.
```
...
pipeline de limpieza ejecutado y verificado (bloque 4 → `empleos_limpio`, §13); `src/`
reestructurado (eliminados `cruzar_datos.py` y `tablas.ipynb`; `limpieza/limpiar_datos.py` y
`limpieza/limpiar_empleos.py`; cuadernos en `notebooks/`, dependencias en `requirements.txt` (raíz)).

**Pendiente principal (bloque 4):** `src/limpieza/limpiar_empleos.py` (clase `LimpiadorEmpleos`):
cargar `data/cruce/empleos.csv` → 1 diagnóstico → 2 duplicados → 3 categorías (re-verificar y
re-categorizar `'None'`) → 4 faltantes (banderas A–C) → 5 outliers (11 filas futuras → eliminar;
conservar cola larga) → 6 validación (tabla §7.7 impresa) → 7 exportar `data/cruce/empleos_limpio.csv`
(solo CSV). Con asserts por fase (nivel "Excepcional") y `requirements.txt`.
...
## 13. EJECUCIÓN DEL BLOQUE 4: `src/limpieza/limpiar_empleos.py` → `empleos_limpio`
...
porqué (A9 del estándar), `requirements.txt` (pandas>=2, pyarrow>=14).
Reproducción: `python src/limpieza/limpiar_empleos.py`.

> Los apartados 13.x describen la **ejecución verificada** del bloque 4 (con números reales). El
> formato de salida pasó de Parquet a **CSV** por decisión del equipo (§16); las cifras no cambian.
...
| 7 Exportar | `data/cruce/empleos_limpio.csv` (~184 MB, **solo CSV**), bitácora de trazabilidad impresa, original intacto | shape final 1.506.434 × 14 |
...
- El original `data/cruce/empleos.csv` **no se modifica** (se mantiene como entrada canónica del bloque 4).
...
- `notebooks/Diagnostico_Limpieza_Empleos.ipynb` reproduce el pipeline: duración **inclusiva** (`+1`),
...
Auditoría del repo contra `docs/BUENAS_PRACTICAS_CODIGO.md` realizada al cerrar la reorganización
...
|---|---|---|
| `README.md` desactualizado (estructura vieja: `src/limpiar_datos.py`, `src/Lectura.ipynb`, sin `notebooks/`, sin `empleos_limpio`) | `README.md` reescrito a la estructura real: tree actualizado, comandos `src/limpieza/…`, notebooks `notebooks/`, datasets con `empleos_limpio.parquet`, decisiones documentadas | `README.md` (raíz) |
| Rutas obsoletas `src/limpiar_*.py` / `src/Lectura.ipynb` | Actualizadas a `src/limpieza/…` y `notebooks/…` en §1, §3, §12 y §13 | Este documento |
| `notebooks/Lectura.ipynb` import roto (`sys.path` apuntaba a `src/` donde ya no vive el código) | CELDA 3 → `sys.path.insert(0, PROYECTO/"src"/"limpieza")`; sintaxis del cuaderno validada (15 celdas, 0 errores) | `notebooks/Lectura.ipynb` |
| Cuaderno Diagnóstico inconsistente con el pipeline: duración exclusiva, `keep=False`, "12 filas", sentinela literal 99999 | Alineado: duración `+1`, `keep='first'`, "11 filas" (3 celdas md + 1 tabla) y constantes `SENTINELA_FIN`; sintaxis validada (67 celdas, 0 errores) | `notebooks/Diagnostico_Limpieza_Empleos.ipynb` |
| `requirements.txt` sin versiones y en carpeta no documentada | Pineado `pandas>=2` / `pyarrow>=14` en `requirements.txt` (raíz); ruta documentada en README y §13 | `requirements.txt` |
| Estándar: sin regla de reproducibilidad, sin regla de documentación, excepción de prints/camelCase implícita, `asserts` del nivel Excepcional no reconocidos, ruta obsoleta `scripts/limpiar_empleos.py` | Añadidas **D5 Reproducibilidad**, **A9 Documentación**, excepciones explícitas en C2 y A1, nota "Excepcional" en D1, D3 marcada opcional, ruta corregida | `docs/BUENAS_PRACTICAS_CODIGO.md` |
| Decisiones "solo parquet" y "prints como entregable" no documentadas formalmente | Documentadas en `README.md` (Decisiones), excepción C2 del estándar y §13.1 | `README.md`, `docs/BUENAS_PRACTICAS_CODIGO.md` |
...
## 15. LIBRO DE ANÁLISIS: `notebooks/presentacion.ipynb` Y `src/filtro/`

Entregable de análisis/mini investigación sobre `data/cruce/empleos_limpio.csv` (solo
lectura; `dur_Q` se **recalcula** con la fórmula inclusiva, no se agrega al CSV). La rúbrica
...
## 16. MIGRACIÓN DEL CRUCE A FORMATO CSV (decisión del equipo)

**Decisión:** el integrado y su limpio se mantienen **solo en CSV** (`data/cruce/empleos.csv`
y `data/cruce/empleos_limpio.csv`). Se eliminaron físicamente los parquet del cruce
(`empleos.parquet`, `empleos_limpio.parquet`); su historial queda en git.

**Qué cambió (2026-09):**

| Artefacto | Antes (Parquet) | Ahora (CSV) |
|---|---|---|
| Entrada del bloque 4 | `empleos.parquet` | `empleos.csv` |
| Salida del bloque 4 | `empleos_limpio.parquet` (~17 MB) | `empleos_limpio.csv` (~184 MB) |
| `limpiar_empleos.py` | lee/escribe parquet | lee y exporta **CSV** (`fase7` ya no escribe parquet) |
| `notebooks/presentacion.ipynb` | `pd.read_parquet(...)` | `pd.read_csv(..., dtype=str, keep_default_na=False, na_values=[""])` + banderas `es_*` → bool |
| `notebooks/Diagnostico_Limpieza_Empleos.ipynb` | carga `empleos.parquet` | carga `empleos.csv` y exporta (comentado) `empleos_limpio.csv` |
| Git | parquet versionados | CSV **no versionados** (`.gitignore`: `data/cruce/*.csv`) |

**Por qué CSV en lugar de Parquet:** formato de interoperabilidad acordado (cualquier
herramienta abre un CSV; los parquet del cruce se usaban porque algún entregable previo los
había adoptado). La pérdida de tipos del CSV no afecta: al leer se restauran
`resume_id` (int), `isco_level` (float) y `es_*` (bool), y se usa `dtype=str` +
`keep_default_na=False` para conservar ceros a la izquierda (`0110`) y no convertir el
literal `'None'` en `NaN`.

**Verificación realizada (post-migración):** `empleos_limpio.csv` == `empleos_limpio.parquet`
(previo) con los nulos normalizados a cadena vacía → **0 celdas diferentes**; shape
1.506.434×14, banderas 104.993/10.176/76.180, `'No reportado'` 174.035, 0 `'None'`, 0 fechas
futuras, `isco_group` conserva `'0110'`. Ejecución del pipeline y de la carga de ambos
cuadernos: sin errores.

**Regeneración si falta un archivo del cruce:** `empleos.parquet` se recupera del historial
git y se exporta a `empleos.csv`; con `data/cruce/empleos.csv` se regenera
`empleos_limpio.csv` vía `python src/limpieza/limpiar_empleos.py`.
...
Cifras verificadas sobre `data/cruce/empleos.csv` y `data/cruce/empleos_limpio.csv`.*