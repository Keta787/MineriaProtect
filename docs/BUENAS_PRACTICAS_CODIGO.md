...
> **Excepción del proyecto (MineriaProtect):** en `notebooks/Diagnostico_Limpieza_Empleos.ipynb` se conservan nombres `camelCase` en español (`calcularDuracionTrimestres`, `contarGruposDuplicados`) por ser los nombres de las funciones requeridas por el entregable de la sesión 3. Es una excepción **explícita y documentada** de ese entregable; el código del proyecto (`src/`) sigue `snake_case`.
...
- Declarar las dependencias del código (p.ej. `requirements.txt`) y documentar el comando de instalación y de ejecución en el `README`.
...
```
CORREGIR:
archivo: src/limpieza/limpiar_empleos.py
